...
static ListPopoverShell create_list_popover_shell(GtkWidget* parent)
{
    ...
    // To get placement right (otherwise will be horizontally misaligned by half the popover size)
    if (gtk_widget_get_direction(GTK_WIDGET(popover)) == GTK_TEXT_DIR_LTR)
    ...
    for (auto const& item : items) {
        item.visit(
            ...
            [&](Web::HTML::SelectItemSeparator const&) {
                ...
                if (i > 0 && i != last_separator_index + 1 && i < static_cast<int>(items.size() - 1)) {
